...
function getArtistByIndex(anArray, aNumber) {
  return `the artist at index ${anArray[aNumber].id} is ${anArray[aNumber].name}`
}
...
function listOfNames(anArray) {
  const artistsCopy = anArray.slice();
  for(let i = 0; i < artistsCopy.length; i++) {
    artistsCopy[i] = artistsCopy[i].name;
  }
  return artistsCopy;
}
...
function removeArtist(anArray, aNumber) {
  const artistsCopy = anArray.slice();
  artistsCopy.splice(aNumber, 1);
  return artistsCopy;
}
...
function addArtist(anArray, name, years, genre, nationality, bio) {
  const newArtist = {
    name: name,
    years: years,
    genre: genre,
...
function lotsOfArt(anArray) {
  ...
  for(let i = 0; i < anArray.length; i++) {
    if(anArray[i].paintings > 100) manyPaintings.push(anArray[i].name);
  }
...
function artistInfo(anArray, name){
  for(let i = 0; i < anArray.length; i++) {
    if(anArray[i].name === name) return anArray[i].bio;
  }
}
...
function artistByCountry(anArray, nationality){
  const artistsWithMatch = [];
  for(let i = 0; i < anArray.length; i++) {
    if(anArray[i].nationality === nationality) artistsWithMatch.push(anArray[i].name);
  }
  return artistsWithMatch;
}
